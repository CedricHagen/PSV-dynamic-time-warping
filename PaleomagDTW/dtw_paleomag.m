function [rid,rii,xc,t_out,Dist,w_scale,overlap]=dtw_v4(x,r,tx,tr,g,edge,flag_norm,flag_rmrpt,qplot)
%%
%Dynamic Time Warping of PSV Data Algorithm
%
%-Outputs-
%rid:    the warped version of r (dec)
%rii:    the warped version of r (inc)
%xc:    correlation between x and r after warping
%Dist:  unnormalized distance between t and r
%w_scale: amount the t is warped
%overlap:  the number of data points that overlap
%
%-Inputs-
%x:     target vector (2 column: dec, inc)
%r:     candidate vector to be warped  --- needs to be the same length as x, can't have NaN's (2 column: dec, inc)
%tx:    time vector for x (taget vector)
%tr:    time vector for r (candidate vector)
%g:     weighting term for going off-diagonal
%edge:  weighting term for the edges
%flag_norm: 1 if you want to normalize the input sections (not-needed for DTW PSV, set at zero) * flag!! Can be removed
...
% Adapted for for PSV data by C.J. Hagen and B.T. Reilly, Winter 2019
...
%         d(:,ind) = d(:,ind) + norm';
           ...
           mu = [tr_fix(ii), tx_fix(ii)];
           Sigma = [tr_fix_sd(ii), tx_fix_sd(ii)];
           [X1, X2] = meshgrid(tr, tx);
           F = mvnpdf([X1(:) X2(:)], mu, Sigma);
           F = -reshape(F, length(tx), length(tr));
           
           % find scaling factor, want minimum to be equal to 95% critical value of d
           F = -(prctile(d(:), 95)/min(F(:)))*F;
...
if 1 % if you don't want to average over the first time points
    ...
    start_t = flipud(start_t); %converts from strat height to strat depth
    start_r1 = flipud(start_r1); %converts from strat height to strat depth
    start_r2 = flipud(start_r2); %converts from strat height to strat depth
...
if qplot,
    ...
    %subplot(num,num,sp(:));
    subplot(5,6,[3 4 5 6 15 16 17 18]); %Heatmap
    cla; hold on;
    imagesc(d); h=colorbar; caxis([0 0.1]) % plot the squared distance matrix, d
    ylabel(h,'Cosine difference'); %font(h,12);
    axis tight; axis off;
    ...
    h=ylabel('stratigraphic depth (m)'); %font(h,12);
    
    set(gca,'ydir','reverse');
    set(gca,'yaxisloc','right');
    set(gca,'xaxisloc','top');
    
    subplot(5,6,[1 13]); %Inclination result
    cla; hold on;
    h=plot(xo(:,2),tx,'r'); set(h,'linewidth',1);  % plot the "known" time series xo
...
%    h=suptitle(['cross-correlation: ',num2str(xc,2),', g: ',num2str(g,2),', edge: ',num2str(edge,2)]); %font(h,12);
    h=xlabel('inclination'); %font(h,12);
    h=ylabel('stratigraphic depth (m)'); %font(h,12);
    set(gca,'xaxisloc','top');
    set(gca,'ydir','reverse');
    set(gca,'XTick',[60 75 90]);
    axis([60,90,min(tx),max(tx)]);
    %font(gca,12);
    
    subplot(5,6,[2 14]); %Declination result
    cla; hold on;
    h=plot(xo(:,1),tx,'r'); set(h,'linewidth',1);  % plot the "known" time series xo
    h=plot(rid,t_out,'k'); set(h,'linewidth',1);  % plot the new warped time series
    axis tight;
    h=xlabel('declination');
    h=ylabel(''); %font(h,12);
    set(gca,'xaxisloc','bottom');
    set(gca,'ydir','reverse');
    set(gca,'YTick',[]);
    set(gca,'XTick',[-180 0 180]);
    axis([-180,180,min(tx),max(tx)]);
    
    subplot(5,6,[21 23]); %Declination orig
    cla; hold on;
    h=plot(tr,ro1,'k'); set(h,'linewidth',1); % plot the original non-warped time series, r0
    axis tight;
    h=ylabel('declination'); %font(h,12);
    %h=xlabel('stratigraphic depth (m)'); %font(h,12);
    %font(gca,12);
    set(gca,'yaxisloc','right');
    set(gca,'ydir','normal');
    set(gca,'XTick',[]);
    set(gca,'YTick',[-180 0 180]);
    axis([min(tr),max(tr),-180,180]);

    subplot(5,6,[27 29]); %Inclination orig
    cla; hold on;
    h=plot(tr,ro2,'k'); set(h,'linewidth',1); % plot the original non-warped time series, r0
    axis tight;
    h=ylabel('inclination'); %font(h,12);
    h=xlabel('stratigraphic depth (m)'); %font(h,12);
    %font(gca,12);
    set(gca,'yaxisloc','right');
    set(gca,'ydir','normal');
    %set(gca,'XTick',[]);
    set(gca,'YTick',[60 75 90]);
    axis([min(tr),max(tr),60,90]);
    drawnow;
    %print(figure(1),'paleomag_heatmap','-dpdf','-bestfit');
